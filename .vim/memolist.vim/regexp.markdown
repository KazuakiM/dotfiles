...
| 構文  | 機能                                                                         |
| ----: | :--------------------------------------------------------------------------- |
| ()    | $nで参照可能。またグループ化で束ねる際にも使用。<br/>Vimの場合は\nで参照可能 |
...
| \z           | 文字列の末尾にのみ一致します。                             |
| (?=pattern)  | 肯定先読み。patternが直後に続くパターンにマッチ。          |
| (?!pattern)  | 否定先読み。patternが直後に続かないパターンにマッチ。      |
| (?<=pattern) | 肯定後読み。patternが直後に続くパターンにマッチ。          |
| (?<!pattern) | 否定後読み。patternが直後に続かないパターンにマッチ。      |
