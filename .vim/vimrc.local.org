...
function! s:system(cmd, message) "{{{
    try
        ...
        echohl WarningMsg | echomsg '['. strftime('%H:%M:%S', localtime()) . '] ' . a:message | echohl None
        set cmdheight=1
    endtry
endfunction "}}}

function! s:BufWritePost(source, dest) abort "{{{
    call s:system('rsync -rlpz --delete --cvs-exclude ' . a:source . ' ' . a:dest, 'Rsync ' . expand('%') . ' Completed!')
endfunction "}}}
...
"autocmd MyAutoCmd BufWritePost /srv/doc/* call s:BufWritePost('/srv/doc/', '/Users/mabuchikazuaki/work/test')
